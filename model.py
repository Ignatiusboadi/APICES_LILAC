    ...
    def __init__(self, in_num_ch=1, num_block=4, inter_num_ch=16, kernel_size=3, conv_act='leaky_relu',
        ...
        for i in range(num_block):
            if i == 0:
                ...
                conv_blocks.append(EncoderBlock3D(inter_num_ch * (2 ** (i - 1)), inter_num_ch, kernel_size=kernel_size,
                                                  conv_act=conv_act, dropout=0, pooling=pooling))
            else:
                conv_blocks.append(EncoderBlock3D(inter_num_ch * (2 ** (i - 1)), inter_num_ch * (2 ** i),
                                                  kernel_size=kernel_size, conv_act=conv_act, dropout=0,
                                                  pooling=pooling))
